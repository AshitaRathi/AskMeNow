...
public class DocumentParserService : IDocumentParserService
{
    ...
    public async Task<List<FAQDocument>> ParseDocumentsFromFolderAsync(string folderPath)
    {
        ...
        // Get all files and filter by supported extensions
    ...
    public async Task<FileProcessingResult> GetFileProcessingStatsAsync(string folderPath)
    {
        ...
        // Try to process supported files to get success/failure counts
        foreach (var filePath in supportedFiles)
        {
            try
            {
    ...
    private async Task<string> ParseJsonFileAsync(string filePath)
    {
        ...
        try
        {
            // Try to parse as JSON and flatten key-value pairs
    ...
    {
        // For .doc files, we'll try to read as text first
        // Note: This is a simplified approach. For better .doc support, consider using a library like Aspose.Words
        try
        {
            return await File.ReadAllTextAsync(filePath, Encoding.UTF8);
        }
        ...
        {
            // If direct text reading fails, return a message indicating the file type
